...
def parseCMDrefine(relioncmd,AZ_letter):
	...
	while counter < tot:
		...
		if l[counter] == '--deleteaws': 
			...
			deleteaws=True
                        continue
		if l[counter] == '--instance': 
			instanceChoice=l[counter+1]
			counter=counter+2
			continue 
		if l[counter] == '--spot': 
			spotprice=float(l[counter+1])
			counter=counter+2
                        continue
		if l[counter] == '--az': 
		...
		newcmd.append(l[counter])
 		counter=counter+1
	return ' '.join(newcmd),selectflag,deleteaws,AZ_letter,instanceChoice,spotprice
...
def relion_refine_mpi(project,AZ_letter):
	...
	relioncmd,select,deleteaws,AZ_letter,instanceSelect,spot=parseCMDrefine(relioncmd,AZ_letter)
	...
	spotaddition=''
	if spot>0: 
		spotaddition='--spotPrice=%f' %(spot)
	#Launch instance
	if os.path.exists('%s/awslog.log' %(outdir)):
		os.remove('%s/awslog.log' %(outdir))
	if 'g3' not in instance: 
		cmd='%s/launch_AWS_instance.py --dirname=%s --instance=%s --availZone=%s%s --volume=%s %s > %s/awslog.log' %(awsdir,data_dir,instance,awsregion,AZ_letter,volID,spotaddition,outdir)
		subprocess.Popen(cmd,shell=True).wait()
	if 'g3' in instance:	
		...
		if awsregion == 'eu-west-1': 
			AMI='ami-2a7bdf53'
                cmd='%s/launch_AWS_instance.py --dirname=%s --AMI=%s --instance=%s --availZone=%s%s --volume=%s %s > %s/awslog.log' %(awsdir,data_dir,AMI,instance,awsregion,AZ_letter,volID,spotaddition,outdir)
                subprocess.Popen(cmd,shell=True).wait()
...
def relion_run_motioncorr(project):
	...
	if len(reliondir) == 0: 
		...
		sys.exit()
	writeToLog('Starting movie alignment in the cloud...','%s/run.out' %(outdir))
	...
	if len(micstar.split('s3-')) > 1:
		...
		if micstarinput[-1] == '/': 
			micstarinput=micstarinput[:-1]
		if len(project) == 0:
                        bucketname='rln-aws-tmp-%s-%s' %(teamname,micstarinput)
			if bucketname[-1] != '/': 
				bucketname=bucketname+'/'
			outputbucketname='%s%s' %(bucketname,outdirname)
		if len(project) > 0:
			bucketname='rln-aws-%s-%s/%s' %(teamname,keyname,micstarinput)
                        outputbucketname='rln-aws-%s-%s/%s/%s' %(teamname,keyname,project,outdirname)	
			if 's3-' in micstar: 
                        	bucketname=micstar.split('s3-')[-1]
		micdir='Micrographs'
		if os.path.exists('%s/s3out.log'%(outdir)):
			os.remove('%s/s3out.log'%(outdir))
		#Check that it exists
	...
	for line in m1:
        ...
        if movieAlignType == 'unblur':
                ...
                if numInstancesRequired > maxx1:
			numInstancesRequired=maxx1
		if awsregion == 'us-west-2':
        	        AMI='ami-28ca2350'
	        if awsregion == 'us-east-2':
        	        AMI='ami-6a17340f'
	...
	while instanceNum < numInstancesRequired:
        	...
        	if os.path.exists('%s/awslog_%i.log' %(outdir,instanceNum)):
                	os.remove('%s/awslog_%i.log' %(outdir,instanceNum))
		if movieAlignType == 'unblur': 
			cmd='%s/launch_AWS_instance.py --AMI=%s --noEBS --dirname=%s --instance=%s --availZone=%s%s  > %s/awslog_%i.log' %(awsdir,AMI,data_dir,instance,awsregion,AZ_letter,outdir,instanceNum)
                        subprocess.Popen(cmd,shell=True)		
		if movieAlignType != 'unblur': 
			cmd='%s/launch_AWS_instance.py --dirname=%s --relion2 --instance=%s --availZone=%s%s --volume=%s > %s/awslog_%i.log' %(awsdir,data_dir,instance,awsregion,AZ_letter,ebsVolList[instanceNum],outdir,instanceNum)
			subprocess.Popen(cmd,shell=True)
		instanceNum=instanceNum+1
       		time.sleep(10)
	instanceNum=0
	...
	if movieAlignType != 'unblur': 
 		writeToLog('Waiting for movie alignment to finish. For %i movies, this job will be finished in approximately %i minutes.' %(movieCounter,int((5*movieCounter)/(numInstancesRequired))+4), '%s/run.out' %(outdir))
	if movieAlignType == 'unblur': 
		writeToLog('Waiting for movie alignment to finish. For %i movies, this job will be finished in approximately %i minutes.' %(movieCounter,int((5*movieCounter)/(numInstancesRequired))+4), '%s/run.out' %(outdir))
	while instanceNum < numInstancesRequired:
		#Create directories on AWS
	...
	while micdone == 0:
		cmd='%s sync rclonename:%s/Micrographs/ %s/Micrographs/ --quiet --transfers %i --max-size 400M > rclone.log' %(rclonepath,outputbucketname.split('s3://')[-1],outdir,math.ceil(numCPUs*2.4))
                writeToLog('waiting ... %s' %(cmd),'%s/run.out' %(outdir))
		subprocess.Popen(cmd,shell=True).wait()
                os.remove('rclone.log')
